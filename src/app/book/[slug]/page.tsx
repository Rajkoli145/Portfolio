...
export async function generateStaticParams() {
  return allChapters.map((chapter) => ({
    slug: chapter._meta.path.replace(/\.(md|mdx)$/, ""),
  }));
}
...
export async function generateMetadata(props: {
  ...
  const chapter = allChapters.find((c) => c._meta.path.replace(/\.(md|mdx)$/, "") === slug);
...
export default async function ChapterPage(props: {
  ...
  const currentIndex = sortedChapters.findIndex(
    (c) => c._meta.path.replace(/\.(md|mdx)$/, "") === slug
  );
    ...
    <div className="max-w-3xl mx-auto py-12 pb-24 sm:py-24 px-6">
      <section id="chapter">
        ...
        <div className="mt-20 pt-8 border-t flex flex-col sm:flex-row justify-between items-center gap-4">
          {previousChapter ? (
            <Link
              href={`/book/${previousChapter._meta.path.replace(/\.(md|mdx)$/, "")}`}
          ...
          {nextChapter ? (
            <Link
              href={`/book/${nextChapter._meta.path.replace(/\.(md|mdx)$/, "")}`}
